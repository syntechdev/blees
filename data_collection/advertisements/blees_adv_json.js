...
noble.on('stateChange', function (state) {
    if (state === 'poweredOn') {
        ...
        console.log("{ \"blees\": \"Scanning started\"}");
        if (blees_choice != null) {
            console.log("{ \"blees\": \"Searching for " + blees_choice + "\"}");
        }
    }
});
...
function parse_blees_data (peripheral, data) {
    ...
    var json_msg = "";
    json_msg += "{ \"blees\": " + "\"" + address + "\", ";
    json_msg += "\"timestamp\": " + "\"" + Date.now()/1000 + "\", ";
    if (sequence_num != -1) {
        console.log('  Sequence Num: ' + sequence_num);
    }
    json_msg += "\"temp\": " + "\"" + temp.toFixed(1)          + "\", ";
    json_msg += "\"humidity\": " + "\"" + humidity.toFixed(1)  + "\", ";
    json_msg += "\"lux\": " + "\"" + light.toFixed(1)          + "\", ";
    json_msg += "\"pressure\": " + "\"" + pressure.toFixed(1)  + "\" ";
    if (imm_accel) {
